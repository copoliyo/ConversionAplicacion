...
public class IndiceFacturasRecibidas {
    ...
    FacturaRecibida facturaRecibida = null;
    ...
    public IndiceFacturasRecibidas() {
        this.strCuenta = "";
    ...
    private void creaGui() {
        ...
        jtFacturasRecibidas.addMouseListener(new TablaListener());
    ...
    public void cargaFacturasRecibidas() {
        ...
        if(strCuenta.length() > 0)
                strSql += " AND FACREC_PROVACRE = '" + strCuenta + "'";
        
        strSql += " ORDER BY FACREC_FECHA ASC";
    ...
    class TablaListener implements ActionListener, MouseListener {
        ...
        public void mouseClicked(MouseEvent arg0) {
            ...
            if (arg0.getClickCount() == 2) {
                ...
                fechaStr = (String) target.getValueAt(row, Columna.FECHA.value);
                fecha = Fecha.cadenaAfecha(fechaStr);
                orden = (Integer) target.getValueAt(row, Columna.ORDEN.value);

                String strSql = "SELECT * FROM FACREC WHERE EMPRESA = '" + DatosComunes.eEmpresa + "' AND "
                        + " FACREC_CENTRO = " + DatosComunes.centroCont + " AND "
                        + " FACREC_FECHA = " + fecha + " AND "
                        + " FACREC_ORDEN = " + orden + " LIMIT 1";

                if (facturaRecibida.read(strSql) == false) {
